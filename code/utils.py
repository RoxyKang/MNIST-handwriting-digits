...
def euclidean_dist_squared(X, Xtest):
    ...
    """
    None: new axis -->
            np.sum(X**2, axis=1)(N,) --> np.sum(X**2, axis=1)[:,None](N,1)
            np.sum(Xtest**2, axis=1)(T,1) --> np.sum(X**2, axis=1)[None](1,T)

    Broadcasting: (N,1) + (1,T) --> (N,T)[with the column repeated for T times] + (N,T)[with the row repeated for N times]
    ...
    np.dot(X,Xtest.T):
            --- N1 ---     |  |      |        N1xT1, N1xT2, ... , N1xTt
            --- N2 ---     |  |  . . |        N2xT1, N2xT2, ... , N2xTt
            ....        x  T1 T2 . . Tt   =   ...
            ....           |  |  . . |        ...
            --- Nn ---     |  |      |        NnxT1, NnxT2, ... , NnxTt
    """
