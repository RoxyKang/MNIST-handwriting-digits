    ...
    def fit(self, X, y):
        ...
        for i in range(epoch):
            ...
            for j in range(iteration):
                mini_batch = np.random.randint(N,size = b_size)
                X_random = X_batch[mini_batch,:]
                y_random = y_batch[mini_batch]
                f,g = self.funObj(weights_flat,X_random,y_random)
                weights_flat = weights_flat - alpha * g
                np.delete(X_batch,mini_batch,0)
                np.delete(y_batch,mini_batch)
        
        weights_flat_new = weights_flat
